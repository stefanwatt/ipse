...
double readUltrasound(double newReading) {
  ...
  if (numUltrasoundReadings % 2 == 0) {
    return (sortedReadings[numUltrasoundReadings / 2 - 1] + sortedReadings[numUltrasoundReadings / 2]) / 2.0;
  } else {
...
void handleUltrasound(unsigned long now)
{
  ...
  distance = readUltrasound(updated_distance);
  ...
  bool shouldToggleDimmingMode = ledOn 
  ...
  bool enteredDimmingRangeRecently = ledOn && isInDimmingRange(distance) && enteredDimmingRange == 0;
  if (enteredDimmingRangeRecently)
  {
    enteredDimmingRangeAtDistance = distance;
    enteredDimmingRange = now;
  }else if (movementDifference > movementTolerance && enteredDimmingRange != 0){
    enteredDimmingRange = now;
    enteredDimmingRangeAtDistance = distance;
  }
